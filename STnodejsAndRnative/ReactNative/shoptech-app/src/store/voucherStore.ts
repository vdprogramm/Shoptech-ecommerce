...
interface VoucherState {
    ...
    validateAndApplyVoucher: (code: string, orderTotal: number) => Promise<{ success: boolean; message: string }>;
...
export const useVoucherStore = create<VoucherState>((set, get) => ({
    ...
    validateAndApplyVoucher: async (code: string, orderTotal: number) => {
        try {
            // Gửi code lên backend kiểm tra tính hợp lệ và lấy thông tin giảm giá
            const response: any = await axiosClient.post('/vouchers/check', { code, orderTotal });
