...
import { voucherService } from "@/lib/api/api-voucher"; // Đường dẫn import api của bạn
...
export const Route = createFileRoute("/_site/admin/vouchers")({
  component: AdminVouchersPage,
});

// Định nghĩa kiểu dữ liệu chuẩn từ Schema MongoDB
interface IVoucher {
  _id: string;
  code: string;
...
function AdminVouchersPage() {
  ...
  // 1. Fetch dữ liệu thật khi load trang
    ...
    <div className="p-4 bg-card rounded-xl border shadow-[var(--shadow-card)]">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Quản lý các chương trình ưu đãi toàn hệ thống ShopTech
          </p>
        </div>
        <button
      ...
      {loading ? (
        <div className="flex justify-center items-center py-20 text-muted-foreground gap-2 text-sm">
          <Loader2 className="w-5 h-5 animate-spin text-primary" /> Đang tải danh sách voucher thực
          tế...
        </div>
      ) : vouchers.length === 0 ? (
        <div className="text-center py-14 text-muted-foreground text-sm border-2 border-dashed rounded-xl">
          Chưa có mã giảm giá nào trên hệ thống. Hãy bấm nút phía trên để tạo.
        </div>
      ) : (
      ...
      {isOpenModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-xs animate-fade-in">
          <div className="bg-card w-full max-w-md rounded-2xl p-6 border shadow-2xl relative">
            <h3 className="text-lg font-bold text-foreground mb-4">Thêm mã giảm giá hệ thống</h3>
