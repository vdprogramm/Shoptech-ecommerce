...
export default function ShipperDeliveryScreen() {
    ...
    const uploadProofImage = async (uri: string) => {
        ...
        try {
            ...
            return response.path || response.data?.path;
        } catch (error) {
        ...
        if (deliveryStatus === 0) {
            ...
            setModalConfig({
                ...
                onConfirm: async () => {
                    ...
                    let imageUrl = null;
                    if (proofImageUri) {
                        imageUrl = await uploadProofImage(proofImageUri);
                    }
                    
                    const success = await updateShipperOrderStatus(order.subOrderId, 'Delivered', imageUrl);
                    setIsUpdating(false);
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                <View style={styles.orderCard}>
                    ...
                    <View style={styles.orderInfoRow}>
                        ...
                        <Text style={styles.infoText}><Text style={styles.infoLabel}>Giao đến: </Text>{order.deliveryAddress}</Text>
                    </View>
                </View>
                    ...
                    <View>
                        {proofImageUri ? (
                            <View style={{ marginBottom: 15, position: 'relative' }}>
                                <Image source={{ uri: proofImageUri }} style={{ width: '100%', height: 200, borderRadius: 8 }} />
                                <TouchableOpacity 
                            ...
                            <TouchableOpacity 
                                style={[styles.actionBtn, { backgroundColor: '#f0f0f0', marginBottom: 15, borderWidth: 1, borderColor: '#ccc', borderStyle: 'dashed' }]} 
                                onPress={handlePickImage}
                            >
                                <Ionicons name="camera-outline" size={24} color="#666" style={{ marginRight: 8 }} />
                                <Text style={[styles.actionBtnText, { color: '#666' }]}>Chụp ảnh minh chứng</Text>
                            </TouchableOpacity>
                        )}

                        <TouchableOpacity 
                            style={[styles.actionBtn, { backgroundColor: proofImageUri ? '#28a745' : '#a5d8b1' }]} 
                            onPress={handleUpdateStatus} 
                            disabled={isUpdating || !proofImageUri}
                        >
                            {isUpdating ? (
                                <ActivityIndicator color="#fff" style={{ marginRight: 8 }} />
                            ) : (
