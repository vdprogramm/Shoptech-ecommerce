...
export default function CheckoutScreen() {
    ...
    const handleApplyVoucher = async (code: string) => {
        if (!code.trim()) {
            Alert.alert('Lưu ý', 'Vui lòng nhập mã giảm giá.');
            return;
        }
        const total = getTotalPrice();
        const res = await validateAndApplyVoucher(code.trim().toUpperCase(), total);
        if (res.success) {
    ...
    const handlePlaceOrder = async () => {
        ...
        const orderData = {
            shippingAddress: defaultAddress._id || defaultAddress.id,
