...
export default function ProductDetailScreen() {
    ...
    const { currentSale, productFlashSale, fetchProductFlashSale } = useFlashSaleStore();
    ...
    let displayStock = selectedVariant ? selectedVariant.stock : product?.stock;
    let isFlashSaleActive = false;

    if (productFlashSale && productFlashSale.isFlashSale && productFlashSale.saleItems) {
        ...
        const flashSaleItem = productFlashSale.saleItems.find((item: any) => 
            item.variantId === targetVariantId || item.variantId === product?._id
        );
        if (flashSaleItem) {
            originalPrice = displayPrice;
            displayPrice = flashSaleItem.salePrice;
            const remainingFlashStock = flashSaleItem.limitStock - flashSaleItem.soldCount;
            displayStock = remainingFlashStock > 0 ? remainingFlashStock : 0;
            isFlashSaleActive = true;
        }
    } else if (currentSale && currentSale.items) {
        const targetVariantId = selectedVariant?._id || (product?.variants?.length > 0 ? (product.variants[0]._id || product.variants[0]) : product?._id);
        ...
        if (flashSaleItem) {
            originalPrice = displayPrice;
            displayPrice = flashSaleItem.salePrice;
            const remainingFlashStock = flashSaleItem.quantityLimit - flashSaleItem.soldCount;
            displayStock = remainingFlashStock > 0 ? remainingFlashStock : 0;
            isFlashSaleActive = true;
        }
    }
    ...
    useEffect(() => {
        ...
        if (product?._id) {
            ...
            fetchAttributes();
            fetchStoreProfile();
            fetchProductFlashSale(product._id);
        }
    }, [product?._id]);
